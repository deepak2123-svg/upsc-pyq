...
function normalize(record: ImportQuestion) {
  ...
  if (record.taxonomyId && !taxonomyNode(record.taxonomyId)) errors.push("taxonomyId is not present in the active canonical registry");
  ...
  return { errors, value: { id: record.id, exam: record.exam, year: typeof record.year === "number" ? record.year : null, paper: typeof record.paper === "string" ? record.paper : null, sourceQuestionNumber: record.questionNumber == null ? null : String(record.questionNumber), subject: record.subject, topic: record.topic, subtopic: typeof record.subtopic === "string" ? record.subtopic : null, taxonomyVersion: record.taxonomyVersion ?? null, taxonomyHead: record.taxonomyHead ?? null, taxonomyChapter: record.taxonomyChapter ?? null, taxonomySubtopic: record.taxonomySubtopic ?? null, taxonomyId: record.taxonomyId ?? null, stem: record.sourceText, promptLines: record.promptLines?.length ? record.promptLines : [record.sourceText], options, correctOption: record.answer, explanation: record.explanation && !/^Explanation pending/i.test(record.explanation) ? record.explanation : null, eliminationNotes: null, origin: record.origin, source: record.source ?? {}, sourceFingerprint: fingerprint, sourceTextHash: record.sourceTextHash ?? createHash("sha256").update(record.sourceText).digest("hex"), sourceTextLocked: record.origin === "pyq", verificationStatus: "unverified", evidence: [], suggestedDifficulty: record.difficulty ?? null, editorialDifficulty: null, workflowStatus: errors.length ? "draft" : "review", requiresFigure: Boolean(record.requiresFigure), figureKey: null } };
}
